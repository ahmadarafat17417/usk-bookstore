                    ...
                    <form action="{{ route('categories.update', $category->id) }}" method="POST" class="px-8 py-8">
                        ...
                        <div class="mb-8">
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="block w-full px-4 py-3 mt-1 border border-gray-300 shadow-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('description', $category->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
